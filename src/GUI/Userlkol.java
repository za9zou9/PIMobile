...
public class Userlkol {
  ...
  Toolbar menu;
    public Userlkol() {
      ...
      System.out.println(ussr.getId());
        f = new Form("Nos Utilisateurs",new BoxLayout(BoxLayout.Y_AXIS));
       menu=f.getToolbar();
   ...
   p.f.add(p.lprenom);
    ...
    AffichageCommentaire aff=new AffichageCommentaire();
     aff.getF().show();
        ...
        ArrayList<User> lis=serviceTask.Userlkol(ussr.getId());
        for(int i=0;i<lis.size();i++){
          
       final User yo=lis.get(i);
      ...
      plus=new Button("Cosnulter "+lis.get(i).getUsername()+" ");
   ...
   plus.addActionListener((e)->{ 
    ...
    AjouterMesAmis dev=new AjouterMesAmis();
  dev.setDesc(""+yo.getUsername());
    dev.setLui(yo);
   dev.getF().show();
   });
